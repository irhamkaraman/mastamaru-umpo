    ...
    public function render()
    {
        ...
        if ($record && ! empty($record->payload_data)) {
            ...
            if (is_array($data)) {
                ...
                $phoneKey = null;
                foreach ($this->mappingRules as $rule) {
                    if ($rule['db_column'] === 'name') {
                        $nameKey = $rule['api_key'];
                    }
                    if ($rule['db_column'] === 'study_program') {
                        $jurusanKey = $rule['api_key'];
                    }
                    if ($rule['db_column'] === 'faculty') {
                        $fakultasKey = $rule['api_key'];
                    }
                    if ($rule['db_column'] === 'phone_number') {
                        $phoneKey = $rule['api_key'];
                    }
                }
                $allNims = [];
