...
class UmpoSyncProgressController extends Controller
{
    ...
    {
        // Inisialisasi status progres awal
        $initialStatus = [
            'status' => 'fetching_api', // fetching_api, processing, completed, error
    ...
    public function executeBatch(Request $request)
    {
        try {
            ...
            // 3. Generate Auth Token dari endpoint generate-token
            ...
            // 4. Ambil Mahasiswa Tahun 2026 dengan Header Auth Token
            ...
            // Pre-fetch existing students in single query for ultra-fast in-memory check
            ...
            // Chunk process (500 records per chunk)
            ...
            foreach ($chunks as $chunkIndex => $chunk) {
                ...
                foreach ($chunk as $mhs) {
                    ...
                    // Fast unique code generator in-memory without database query
                ...
                // Batch upsert to database in a single super-fast query
                if (!empty($upsertData)) {
                    Attendance::upsert(
                        $upsertData,
                        ['student_id'], // Unique index
                        ['name', 'study_program', 'faculty', 'phone_number', 'updated_at'] // Update columns
                    );
                }
                ...
                // Update realtime status in cache
                Cache::put($this->progressKey, [
                    'status' => 'processing',
